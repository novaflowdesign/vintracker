...
1. Card name — printed in the top-left area.
   ...
   - Japanese card: the main Pokémon name is the LARGE katakana/kanji text printed in the upper-right area of the card header, always accompanied by an HP value (e.g. "230HP") to its right. That is the only name you should use. There is also a small circular thumbnail on the left side of the header showing the pre-evolution Pokémon — completely ignore that image and any text associated with it. Translate the large main name to the OFFICIAL ENGLISH Pokémon name (e.g. ピカチュウ → "Pikachu", リザードン → "Charizard", ゲッコウガ → "Greninja"). Include the card type suffix printed next to that main name if visible (e.g. "ex", "V", "VMAX", "VSTAR", "GX").
